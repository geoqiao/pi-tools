...
| 执行效率 | Code Mode 如何组织调用，可能影响多少成本？ | 直接调用反事实情景、Token / 费用差额及假设敏感性；批处理覆盖率、均值 / 中位数 / P75、趋势与证据覆盖率 |
...
在“执行效率”中，对比 **同一批可估响应** 的日志 Token、按当前费率重算的费用，以及假设将内层工具展开为直接调用后的 Token / 费用。差额为 **直接调用 − Code Mode**：正值表示本情景中 Code Mode 较省，负值表示较贵。
